...
function _ilGenerateFallbackIdeas(topic, audience, type, goal, source) {
    const t = Date.now();
    const grp = (fmt) => fmt.includes('video') || fmt.includes('YouTube') ? 'Video' : fmt.includes('Blog') ? 'Blog' : 'Social';
    _ilIdeas = [
        _ilMakeIdea(t+'a', `5 costly ${topic} mistakes that ${audience} make before ₹1Cr`, type, grp(type), audience, `Planning ${topic.toLowerCase()}? These 5 mistakes cost Indian investors ₹5-15L in taxes and missed returns every year.`, 'Mistakes to avoid', 91, 'Book a free portfolio review with Right Horizons advisors.', '7-slide carousel: bold mistake headline per slide, red/green contrast, ₹ impact callout boxes, RH brand navy + indigo palette.', 'Add SEBI IA disclaimer. Avoid guaranteed return language. Use "past performance does not guarantee future returns" where needed.'),
        _ilMakeIdea(t+'b', `${topic} checklist: 8 steps every ${audience} must complete`, 'LinkedIn carousel', 'Social', audience, `Before you invest another rupee, run through this ${topic.toLowerCase()} checklist. Most ${audience.toLowerCase()} skip step 4.`, 'Checklist', 88, 'Download the full checklist — link in comments. Or book a free 30-min call with our advisory team.', '10-slide carousel: numbered checklist cards, checkmark icons, calm navy/teal palette, final slide = CTA with QR code.', 'Keep educational. Do not promise specific returns. Reference Section 80C/80D limits where relevant.'),
        _ilMakeIdea(t+'c', `"${topic} is only for the rich" — and 4 other myths ${audience} believe`, 'Short video', 'Video', audience, `₹5,000/month SIP can build ₹1Cr+ in 15 years. Yet most ${audience.toLowerCase()} think ${topic.toLowerCase()} is only for HNIs. Let's bust 5 myths.`, 'Myth-busting', 86, 'Follow Right Horizons for weekly myth-busting. Book a free consultation — link in bio.', 'Speaker-led 45-sec Reel: on-screen MYTH/FACT labels, pop-up ₹ figures, split-screen format, captions mandatory.', 'Avoid personalized financial advice. Add "consult your financial advisor" disclaimer. Use SEBI-compliant language.'),
        _ilMakeIdea(t+'d', `The complete ${topic} guide for ${audience} in 2025 (with ₹ examples)`, 'Blog', 'Blog', audience, `This 2,000-word guide covers everything ${audience.toLowerCase()} need to know about ${topic.toLowerCase()} — with real ₹ calculations, tax-saving sections, and step-by-step action plans.`, 'Educational', 89, 'Ready to act on this? Schedule a free portfolio review with Right Horizons — no obligation.', 'Long-form blog: H2/H3 structure, comparison tables with ₹ figures, embedded calculators, FAQ section targeting Google PAA, internal links to RH services.', 'Reference specific tax sections (80C, LTCG at 12.5%). Use "subject to market risks" disclaimer. Avoid forward-looking guarantees.'),
        _ilMakeIdea(t+'e', `Why ${audience} should review ${topic.toLowerCase()} before March 31`, 'Social post', 'Social', audience, `March 31 is 90 days away. If you haven't reviewed your ${topic.toLowerCase()} strategy, you could miss ₹1.5L in tax savings under Section 80C alone.`, 'Timely insight', 84, 'DM us "REVIEW" or book a slot at righthorizons.com — free tax-aware portfolio check.', 'Single image: calendar urgency visual, ₹ savings highlight, RH brand colors, clean sans-serif typography with bold stat callout.', 'Use cautious language: "up to ₹1.5L" not guaranteed savings. Add standard investment disclaimer.'),
        _ilMakeIdea(t+'f', `${topic} explained in 60 seconds — what every ${audience.split(' ')[0]} should know`, 'YouTube video', 'Video', audience, `If ${topic.toLowerCase()} feels complicated, here are the 3 things that actually matter — explained with real ₹ numbers in under a minute.`, 'Quick explainer', 85, 'Subscribe for weekly finance explainers. Join our next live webinar — registration link in description.', 'Animated explainer: 3 numbered points with ₹ callouts, Nifty 50 chart reference, clean motion graphics, CTA end-card with webinar QR code.', 'Keep guidance educational, not advisory. Do not reference specific stocks or funds. Add SEBI registration number in description.')
    ];
}
...
function _ilRenderSeasonal() {
    const data = [
        ['Union Budget 2025: What changes for your ₹1Cr+ portfolio', 'Blog + carousel', 'Break down Budget tax changes (LTCG, 80C, new slabs) into actionable steps. Use before/after ₹ comparison tables. Lead with "Here\'s what changed for investors earning ₹15L+" — not generic news.'],
        ["Father's Day: '3 money lessons my father never had access to'", 'Social creative', 'Emotional hook tied to generational wealth building. Compare ₹1L invested in 1995 vs today. Keep brand-safe with RH advisory CTA. Works for LinkedIn + Instagram.'],
        ['March 31 countdown: ₹1.5L tax savings you\'re about to lose', 'Email + LinkedIn', 'Urgency-driven campaign. Checklist: 80C (₹1.5L), 80D (₹25K-₹1L), NPS 80CCD(1B) (₹50K). Position RH as the advisor who reminds you in time. Include booking CTA.'],
        ['Independence Day: "What does financial independence cost in India?"', 'Carousel', 'Calculate ₹ corpus needed for financial independence at 45/50/55 across Tier 1/2 cities. Use SWP math. Tie to RH retirement planning services — not forced patriotism.'],
        ['Diwali: The ₹5 wealth moves every family should make this Dhanteras', 'Social + email', 'Gold vs equity vs SIP comparison with ₹1L/year examples over 10 years. Family gifting tax rules (₹50K exemption). Position RH for year-end portfolio review.'],
        ['NRI summer return: "Before you land in India, fix these 5 things"', 'Webinar promo', 'Target NRIs visiting June-August. Cover: DTAA benefits, ₹15L+ income reporting, NRO/NRE repatriation, capital gains on Indian property. Drive to RH NRI advisory webinar.']
    ];
...
async function generateILWebinarIdeas() {
    ...
    const items = [
        ...
        ['Email', 'Webinar follow-up: "Here\'s what you missed" nurture email', '3-part email: recap key insights with ₹ examples, link to webinar replay, CTA for free portfolio review. Subject line A/B test suggestions included.'],
        ['Quote card', 'Expert quote graphic with key stat from the session', 'Pull the strongest expert quote. Design as branded social graphic with ₹ stat overlay, speaker photo, and Right Horizons logo. Use for LinkedIn + Instagram.']
    ];
    if (kpis) kpis.innerHTML = '<span class="il-kpi">1 LinkedIn post</span><span class="il-kpi">1 Carousel</span><span class="il-kpi">1 Short video</span><span class="il-kpi">1 Blog</span><span class="il-kpi">1 Email</span><span class="il-kpi">1 Quote card</span>';
